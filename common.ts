...
import { readableStreamToString, BrexExpensesError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new BrexExpensesError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new BrexExpensesError(e, e.response?.data)
                }
                if (innerError instanceof BrexExpensesError) {
                    // Got "BrexExpensesError" from the above try block
                    throw innerError;
                }
